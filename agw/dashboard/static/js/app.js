    ...
    _this = L.Common = {
        ...
        initViewAndDownloadEvent: function (type) {
            ...
            if (type == "oauth2") {
                ...
                data = L.Balancer.data;
            } else if(type == "limiting_rate") {
                data = L.Limiting_rate.data;
            } else {
                return;
            }

            $("#view-btn").click(function () {//视图转换
                var self = $(this);
        ...
        initSwitchBtn: function (type) {
            ...
            if (type == "oauth2") {
                ...
                op_type = "/api/balancer";
                name = '路由负载';
            } else if(type == 'limiting_rate') {
                op_type = "/api/Limiting_rate";
                name = '限速';
            } else {
                return;
            }
                                ...
                                $.ajax({
                                    ...
                                    success: function (result) {
                                        if (result.success) {
                                            ...
                                            self.find("span").text("启用" + name);

                                            return true;
                                        } else {
                                            L.Common.showErrorTip("提示", result.msg || "关闭" + name + "发生错误");
                                            return false;
                                        }
                                    },
                                    error: function () {
                                        L.Common.showErrorTip("提示", "关闭" + name + "请求发生异常");
                                        return false;
                                    }
                                });
                            }
                                ...
                                $.ajax({
                                    ...
                                    success: function (result) {
                                        if (result.success) {
                                            ...
                                            self.find("span").text("停用" + name);

                                            return true;
                                        } else {
                                            L.Common.showErrorTip("提示", result.msg || "开启" + name + "发生错误");
                                            return false;
                                        }
                                    },
                                    error: function () {
                                        L.Common.showErrorTip("提示", "开启" + name + "请求发生异常");
                                        return false;
                                    }
                                });
                            }
        ...
        initRuleEditDialog: function (type, context) {
            var op_type = "";
            var rules_key = "";
            var edit_name = (typeof(arguments[2]) != 'undefined') ? arguments[2] : '规则';
            if (type == "oauth2") {
                op_type = "/api/oauth2";
            } else if (type == 'balancer') {
                op_type = "/api/balancer";
            ...
            $(document).on("click", ".edit-btn", function () {
                ...
                if (!rule_id || !rule) {
                    L.Common.showErrorTip("提示", "要编辑的"+edit_name+"不存在或者查找出错");
                    return;
                }
                                ...
                                $.ajax({
                                    ...
                                    success: function (result) {
                                        if (result.success) {
                                            ...
                                            L.Common.showErrorTip("提示", result.msg || "编辑"+edit_name+"发生错误");
                                            return false;
                                        }
                                    },
                                    error: function () {
                                        L.Common.showErrorTip("提示", "编辑"+edit_name+"请求发生异常");
                                        return false;
                                    }
                                });
        ...
        initRuleDeleteDialog: function (type, context) {
            ...
            $(document).on("click", ".delete-btn", function () {
                ...
                var rule_id = $(this).attr("data-id");
                var content = (type=='oauth2') ? "确定要删除账户名称为【" + name + "】的oauth2吗？" : "确定要删除规则【" + name + "】吗？";
                var d = dialog({
                    title: '提示',
                    width: 480,
                    content: content,
        ...
        //ajax获取分页数据
        pagerAjaxBtn: function (tpl_id, html_id, highlight_id) {
            $(document).on("click", "#pager_ajax", function () {
