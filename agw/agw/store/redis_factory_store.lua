        ...
        self.construct = function(_, h, name)
            ...
            for _, v in pairs(h.commands) do
                self[v] = function(self, ...)
                    ...
                    if (type(self.redis_instance) == cjson.null or type(self.redis_instance) == 'nil') then
                        local ok
...
-- config example
-- local config = {
--     redis_main = { -- your connection name 
--         host = '127.0.0.1',
--         port = 6379,
--         pass = '',
...
--     }
-- }
-- local redis_factory = require("agw.lib.store.redis_factory_db")(config)
-- local ok, redis_main = redis_factory:spawn('redis_main')
-- local ok = redis_main:set('test', "aaaaaaaaaaa")
