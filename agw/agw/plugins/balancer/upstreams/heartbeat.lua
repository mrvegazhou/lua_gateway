...
local log           = ngx.log
local localtime     = ngx.localtime --返回nginx当前时间缓存，本地时间，格式yyyy-mm-dd hh:mm:ss
local ERR           = ngx.ERR
local WARN          = ngx.WARN
local now           = ngx.now
...
local function update_peer_status(srv, sensibility)
    ...
    if status == _M.STATUS_OK then
        ...
        if old_status.status ~= status and old_status.fail_num >= sensibility then
...
local function cluster_heartbeat(skey)
    ...
    if ups.enable == false or (ups.enable == nil and base.upstream.default_heartbeat_enable == false) then
        return
    end
    ...
    for level, cls in pairs(ups.cluster) do
        for _, srv in ipairs(cls.servers) do
            ...
            local cb_heartbeat = ups_heartbeat or heartbeat[ups_typ] or heartbeat["general"]
